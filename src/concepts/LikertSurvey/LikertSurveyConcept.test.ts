...
Deno.test(
  "Principle: Author creates survey, respondent answers, author views results",
  async () => {
    ...
    try {
      ...
      const surveyResponses = await surveyConcept._getSurveyResponses({
        survey,
      });
      assertEquals(
...
Deno.test("Action: submitResponse requirements are enforced", async () => {
  ...
  try {
    ...
    const { question } = (await surveyConcept.addQuestion({
      survey,
      text: "A question",
    })) as {
      question: ID;
...
Deno.test(
  "Action: updateResponse successfully updates a response and enforces requirements",
  async () => {
    const [db, client] = await testDb();
    const surveyConcept = new LikertSurveyConcept(db);
    try {
